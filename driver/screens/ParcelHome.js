...
const ParcelHome = () => {
  ...
  const fetchUserDetails = async () => {
    ...
    try {
      ...
      if (response.data.success && response.data.partner) {
        ...
        const dashboard = await axios.get(`https://www.appv2.olyox.com/api/v1/rides/parcelDashboardData/${partnerData?._id}`);
  ...
  const updateDriverLocation = useCallback(async (latitude, longitude) => {
    try {
      await axios.post('https://www.appv2.olyox.com/webhook/cab-receive-location', {
