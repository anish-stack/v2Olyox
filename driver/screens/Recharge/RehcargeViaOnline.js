...
export default function RechargeViaOnline() {
  ...
  const fetchUserDetails = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.1.37:3100/api/v1/rider/user-details", {
  ...
  const initiatePayment = async () => {
    ...
    setLoading(true)
    const baseUrl = `http://192.168.1.37:3100/api/v1/rider/recharge-wallet/${selectedMemberId}/${userData?.BH}`
    ...
    try {
      ...
      const verifyResponse = await axios.post(
        `http://192.168.1.37:3100/api/v1/rider/recharge-verify/${userData?.BH}`,
        {
